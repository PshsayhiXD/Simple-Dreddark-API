...
(async () => {
  ...
  const debug = {
    ...
    _moduleStyle(level) {
      const bg =
        level === "LOG" ? "#e8f0fe" : level === "WARN" ? "#fef7e0" : "#fce8e6";
      const fg =
        level === "LOG" ? "#1967d2" : level === "WARN" ? "#b06000" : "#c5221f";
      return [
        "background:" + bg,
        "color:" + fg,
        "padding:2px 9px",
    ...
    group(moduleName, label) {
      ...
      this.useGroups
        ? root.console.group(fmt, ...styles)
        : root.console.log(fmt, ...styles);
    },
    groupCollapsed(moduleName, label) {
      ...
      const styles = name
        ...
        : [badgeCss, this._metaStyle()];
      this.useGroups
        ? root.console.groupCollapsed(fmt, ...styles)
        : root.console.log(fmt, ...styles);
    },
    groupEnd() {
      if (!this.enabled) return;
      if (this.useGroups) root.console.groupEnd();
    },
    log(moduleName, ...rest) {
      if (typeof moduleName !== "string")
        return this._print("LOG", null, [moduleName, ...rest]);
      this._print("LOG", moduleName.trim(), rest);
    },
    warn(moduleName, ...rest) {
      if (typeof moduleName !== "string")
        return this._print("WARN", null, [moduleName, ...rest]);
      this._print("WARN", moduleName.trim(), rest);
    },
    error(moduleName, ...rest) {
      if (typeof moduleName !== "string")
        return this._print("ERR", null, [moduleName, ...rest]);
      this._print("ERR", moduleName.trim(), rest);
    },
      ...
      emit(type, payload) {
        (map[type] || []).forEach((fn) => {
          try {
            ...
            debug.error("event", "event handler error", e);
          }
        });
      },
    });
  ...
  const client = (() => {
    ...
    const _stat = {
      ...
      role: null, // "guest" | "crew" | "captain" | null
      rank: null, // 0 | 1 | 3 | null
      isRegistered: null, // boolean | null
      isCaptain: null, // boolean
    };
    accountInfo().then((info) => {
      if (!info) return;
      _stat.name = info.name;
      _stat.isRegistered = info.isRegistered;
    });
    const stat = async (key) => {
      _stat.isCaptain = await ship.isClientCap();
      const snap = {..._stat};
      if (key === undefined || key === null) return snap;
      if (Array.isArray(key))
        return Object.fromEntries(key.map((k) => [k, snap[k] ?? null]));
  ...
  const utils = (() => {
    ...
    const state = (initial = {}) => {
      ...
      const snapshot = () => ({...data});
      return Object.freeze({get,set,watch,snapshot});
    };

    const currentReplaceState = history.replaceState.bind(history);
    let replacedState = false;

    const freezeReplaceState = () => {
      if (replacedState) return;
      Object.defineProperty(history,"replaceState",{value:()=>{},writable:false,configurable:true});
      replacedState = true;
    };
    const restoreReplaceState = () => {
      if (!replacedState) return;
      Object.defineProperty(history,"replaceState",{value:currentReplaceState,writable:true,configurable:true});
      replacedState = false;
    };
  ...
  const ship = (() => {
    ...
    const getAllShipPlayer = async () => {
      ...
      /* EXTERNAL */ const clientName = await client.getClientUsername();
    ...
    const getShipFromLink = async (
      link,
      token = "fZEBE7fIFigqHKHPHiAzp0SW",
    ) => {
      try {
    ...
    const parseInvite = (invite) => {
      ...
      const v = invite.trim();
      const m1 = v.match(
        /^https?:\/\/(?:test\.)?drednot\.io\/invite\/([a-z0-9_-]+)$/i,
      );
      if (m1)
        return {
          code: m1[1],
    ...
    const getCurrentJoinedShip = () =>
      storage.defaultStorage.get("ship.current") || null;

    const ensureCrewControl = async () => {
      if (!teamManagerBtn || !teamMenu) return false;
      if (teamMenu.classList.contains("hidden")) teamManagerBtn.click();
      const crewBtn = [...teamMenu.querySelectorAll("button")].find((b) =>
        /crew control/i.test(b.textContent),
      );
    ...
    const coolsnake303 = async (direction) => {
      ...
      const inp = document.querySelector(
        "#team_players input[placeholder='Search'],#team_players input",
      );
      if (!inp) return false;
      ...
      try {
        await dom.waitUntil(
          () =>
            [...document.querySelectorAll("#team_players table tbody tr")].some(
              (r) => /coolsnake303/i.test(r.textContent),
            ),
          2000,
        );
      } catch {
      ...
      const row = [
        ...document.querySelectorAll("#team_players table tbody tr"),
    ...
    const initSaveJoinedShip = () => {
      ...
      shipyard.addEventListener("click", (e) => {
        ...
        storage.defaultStorage.set("ship.current", {
          id,
          title,
          crew,
    ...
    const clearCurrentShip = () => {
      storage.defaultStorage.delete("ship.current");
    };
    ...
    const logs = (() => {
      debug.forceWarn(
        "ship.logs",
        "BETA;",
        "ship.logs are in beta phase, expect bug. use at your own risk !",
      );
        ...
        emit(evt, data) {
          ...
          for (let i = 0; i < arr.length; i++) {
            try {
              ...
              debug.error("events", "event handler error", e);
            }
          }
        },
      };
      ...
      const validateCallback = (fn, name) => {
        if (typeof fn !== "function")
          throw new TypeError(
            `${name || "callback"} must be a function, got ${typeof fn}`,
          );
        return fn;
      };
      ...
      const validateString = (str, name, required = true) => {
        ...
        if (str !== null && str !== undefined && typeof str !== "string")
          throw new TypeError(`${name} must be a string, got ${typeof str}`);
        return str;
      };
      ...
      const validateRegex = (regex, name) => {
        ...
        if (typeof regex === "string") {
          try {
            ...
            throw new Error(`${name} regex is invalid: ${e.message}`);
          }
        }
        if (regex instanceof RegExp) return regex;
        throw new TypeError(
          `${name} must be a RegExp or string, got ${typeof regex}`,
        );
      };

      const validateRule = (rule, name = "rule") => {
        if (!rule || typeof rule !== "object")
          throw new TypeError(`${name} must be an object, got ${typeof rule}`);
        if (!rule.name || typeof rule.name !== "string")
          throw new Error(`${name}.name must be a non-empty string`);
        if (rule.name.length > 100)
          throw new Error(`${name}.name too long (max 100 chars)`);
        validateRegex(rule.regex, `${name}.regex`);
        if (rule.mapper && typeof rule.mapper !== "function")
          throw new TypeError(`${name}.mapper must be a function or undefined`);
        return rule;
      };
      ...
      const isTimestampOnly = (txt) => {
        if (!txt) return false;
        const t = txt.trim();
        if (/^\d{1,2}:\d{2}:\d{2}\s+\d{1,2}\/\d{1,2}\/\d{4}$/.test(t))
          return true;
      ...
      const defaultRules = [
        ...
        },
        {
          name: "used_item",
          regex: /^\s*used\s+(.+?)\.?$/i,
          mapper: (m) => ({action: "used", target: m[1].trim()}),
        },
        {
          name: "placed",
          regex: /^\s*(?:placed|placed a|placed an)\s+(.+?)\.?$/i,
          mapper: (m) => ({action: "placed", target: m[1].trim()}),
        },
        {
          name: "started_using",
          regex: /^\s*(?:started using|started)\s+(.+?)\.?$/i,
          mapper: (m) => ({action: "started", target: m[1].trim()}),
        },
        {
          name: "set_destination",
          regex: /set the destination zone to:\s*(.+?)\.?$/i,
          mapper: (m) => ({action: "set_destination", target: m[1].trim()}),
        },
        {
          name: "teleport",
          regex: /teleported the ship to\s*(.+?)\.?$/i,
          mapper: (m) => ({action: "teleport", target: m[1].trim()}),
        },
        {
          name: "renamed_ship",
          regex: /^\s*renamed\s+ship\s+to\s+"?(.+?)"?\.?$/i,
          mapper: (m) => ({action: "renamed", target: m[1].trim()}),
        },
        {
          name: "edited_motd",
          regex: /^\s*edited\s+the\s+ship['']s\s+MOTD\.?$/i,
          mapper: (m) => ({action: "edited_motd", target: "MOTD"}),
        },
        {
          name: "demoted",
          regex: /^\s*demoted\s+(.+?)\s+to\s+(.+?)\.?$/i,
          mapper: (m) => ({
            action: "demoted",
            target: m[1].trim(),
            method: m[2].trim(),
        ...
        },
        {
          name: "banned",
          regex: /^\s*banned\s+(.+?)\.?$/i,
          mapper: (m) => ({action: "banned", target: m[1].trim()}),
        },
        {
          name: "kicked",
          regex: /^\s*kicked\s+(.+?)\.?$/i,
          mapper: (m) => ({action: "kicked", target: m[1].trim()}),
        },
        {
          name: "emergency_warp",
          regex: /^\s*(cancelled|initiated)\s+emergency\s+warp\.?$/i,
          mapper: (m) => ({
            action: m[1].toLowerCase(),
            target: "emergency warp",
          }),
        },
        {
          name: "ship_verb",
          regex:
            /^\s*(joined|left|loaded|saved|unloaded)\s+(?:a\s+|an\s+|the\s+)?(.+?)\.?$/i,
          mapper: (m) => ({action: m[1].toLowerCase(), target: m[2].trim()}),
        },
        {
          name: "joined",
          regex: /^\s*joined(?:\s+(.+?))?\.?$/i,
      ...
      const parseActionDetails = (text) => {
        ...
        if (!raw)
          return {
            matchName: null,
            parsed: {action: null, method: null, target: null, raw},
          };
        ...
        return {
          matchName: null,
          parsed: {action: raw, method: null, target: null, raw},
        };
      };
      ...
      const parseParagraph = (p) => {
        try {
          ...
          if ((!matchName || matchName === "simple_verb") && !isTimestamp) {
            ...
            if (unknown[parsed.raw] >= AUTO_LEARN_THRESHOLD) {
              ...
              if (first) {
                ...
                const ruleName =
                  "auto_" + verb + "_" + hash(parsed.raw).slice(0, 6);
                const regex = new RegExp(
                  "^\\s*" + escapeRegex(verb) + "(?:\\s+(.+))?\\s*\\.?$",
                  "i",
                );
                rules.push({
                  name: ruleName,
                  ...
                  mapper: (m) => ({
                    action: verb,
                    target: m[1] ? m[1].trim() : null,
                  }),
                });
                unknown[parsed.raw] = 0;
                events.emit("learned:rule", {
                  ruleName,
                  verb,
                  example: parsed.raw,
                });
          ...
          return {
            ...
            matchRule: matchName,
            actionRaw: parsed.raw,
            timeRaw: italic ? italic.innerText.trim() : null,
            timeIso,
            rawText,
            rawHtml,
      ...
      const appendLogs = (items) => {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (!hashes.has(parsed.id)) {
            ...
            if (parsed.method)
              events.emit("log:method:" + parsed.method, parsed);
            if (parsed.matchRule)
              events.emit("log:match:" + parsed.matchRule, parsed);
            events.emit("log:action", {
              action: parsed.action,
              method: parsed.method,
              target: parsed.target,
              user: parsed.actor,
              timestamp: parsed.timeIso || parsed.capturedAt,
              raw: parsed.actionRaw,
            });
          }
        }
        if (appended.length) {
          ...
          window.dispatchEvent(
            new CustomEvent("drednotActionTrainerNewLogs", {
              detail: {newCount: appended.length, entries: appended},
            }),
          );
        }
        return appended;
      ...
      const processExisting = (container) => {
        const parsed = Array.from(container.querySelectorAll("p"))
        ...
        on(a, b) {
          try {
            if (typeof a === "function" && !b) {
              ...
              events.on("log", a);
              setTimeout(
                () => logEntries.filter((e) => !e.isTimestamp).forEach(a),
                0,
              );
              return () => events.off("log");
            }
            if (typeof a === "string" && typeof b === "function") {
              ...
              if (str.indexOf(" ") !== -1) {
                const wrapper = (entry) => {
                  if (matchPattern(str, entry)) b(entry);
                };
                events.on("log", wrapper);
                setTimeout(
                  () =>
                    logEntries
                      .filter((e) => !e.isTimestamp && matchPattern(str, e))
                      .forEach(b),
                  0,
                );
              ...
              if (str === "log")
                setTimeout(
                  () => logEntries.filter((e) => !e.isTimestamp).forEach(b),
                  0,
                );
              if (str === "log:action")
                setTimeout(
                    ...
                    logEntries
                      ...
                      .forEach((e) =>
                        b({
                          ...
                          target: e.target,
                          user: e.actor,
                          timestamp: e.timeIso || e.capturedAt,
                          raw: e.actionRaw,
                        }),
                      ),
                  0,
        ...
        onVerb(verb, h) {
          try {
            ...
            setTimeout(
              () =>
                logEntries
                  .filter((e) => !e.isTimestamp && e.action === verb)
                  .forEach(h),
              0,
            );
        ...
        onMethod(meth, h) {
          try {
            ...
            setTimeout(
              () =>
                logEntries
                  .filter((e) => !e.isTimestamp && e.method === meth)
                  .forEach(h),
              0,
            );
        ...
        onMatch(ruleName, h) {
          try {
            ...
            setTimeout(
              () =>
                logEntries
                  .filter((e) => !e.isTimestamp && e.matchRule === ruleName)
                  .forEach(h),
              0,
            );
        ...
        addRule(rule) {
          try {
            ...
            const r = {
              ...
              mapper:
                typeof rule.mapper === "function"
                  ? rule.mapper
                  : (m) => ({
                      action: (m[1] || m[0] || "").toLowerCase(),
                      target: m[2] ? m[2].trim() : null,
                    }),
            };
  ...
  const commands = (() => {
    ...
    const ACHook = async () => {
      ...
      try {
        ...
        debug.error("autoComplete", "could not find chat elements", e);
        return;
      }
      const showContainer = () => {
        autocomplete.style.removeProperty("display");
        autocomplete
        .closest("#chat-autocomplete-container")
      ...
      chatInp.addEventListener("input", () => {
        ...
        .filter((entry) => {
          ...
          if (typeof entry.visible === "function" && !entry.visible(val))
            return false;
          return true;
        })
            ...
            if (entry.runOnClick) {
              ...
              if (typeof entry.onExecute === "function") {
                try {
                  ...
                  debug.error("autoComplete", "onExecute threw", e);
                }
              } else Dreddark.chat.send(resolvedCmd);
            } else {
    ...
    const chatHandler = (e) => {
      for (const name in map) {
        ...
        if (c.sessionCooldown) {
          const until = storage.defaultStorage.get(cdNs, userKey) || 0;
          if (until > now()) {
        ...
        if (c.persistCooldown) {
          const until = storage.defaultStorage.get(cdNs, userKey);
          if (until > now()) {
            error(`cooldown ${((until - now()) / 1000) | 0}s`);
            return;
          }
        ...
        if (c.globalCooldown) {
          const until = storage.defaultStorage.get(cdNs, globalKey);
          if (until > now()) {
        ...
        c.run(e, args);
        if (c.sessionCooldown)
          storage.defaultStorage.set(cdNs, userKey, now() + c.sessionCooldown);
        if (c.persistCooldown)
          storage.defaultStorage.set(cdNs, userKey, now() + c.persistCooldown);
        if (c.globalCooldown)
          storage.defaultStorage.set(cdNs, globalKey, now() + c.globalCooldown);
        return;
      }
    };
    return Object.freeze({
      ...
      registerCommandAutoComplete(label, opts = {}) {
        if (typeof label !== "string" || !label)
          throw new Error(
            "registerCommandAutoComplete: label must be a non-empty string",
            );
        if (opts.rolePermission && !(opts.rolePermission in rankValue))
          throw new Error(
            `registerCommandAutoComplete: invalid rolePermission "${opts.rolePermission}". must be one of: ${Object.keys(rankValue)
              .filter((k) => isNaN(k))
              .join(", ")}`,
              );
  ...
  const pvpEvent = (() => {
    ...
    const pvpEventFn = async (eventQuery) => {
      await fetchSchedule();
      if (typeof eventQuery === "object" && !Array.isArray(eventQuery))
        eventQuery = JSON.stringify(eventQuery);
      else if (Array.isArray(eventQuery)) eventQuery = eventQuery.join(" ");
      if (typeof eventQuery === "string") eventQuery = eventQuery.toLowerCase();
      const now = new Date();
      if (eventQuery === pvpEventQuery.all())
        return schedule.map(({ date }) => ({ date }));
      if (eventQuery === pvpEventQuery.today()) {
      ...
      if ([pvpEventQuery.next(), pvpEventQuery.second(), pvpEventQuery.third(), pvpEventQuery.fourth()].includes(eventQuery)) {
        const upcoming = schedule
        .filter(({ date }) => new Date(date) > now)
        .sort((a, b) => new Date(a.date) - new Date(b.date));
        const index =
        eventQuery === pvpEventQuery.next()
        ? 0
        : eventQuery === pvpEventQuery.second()
        ? 1
        : eventQuery === pvpEventQuery.third()
        ? 2
        : 3;
      ...
      if (pvpEventQuery.week.includes(eventQuery)) {
        const dayEvents = schedule.filter(
          ({ date }) => new Date(date).getDay() === pvpEventQuery.week.indexOf(eventQuery),
          );
        return dayEvents.length
        ? dayEvents.map(({ date }) => date)
        : Error(`No events found for ${eventQuery}`);
      }

      // TODO: Remove this check in a future release once past-date querying is supported.
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      if (new Date(eventQuery) < today)
        return Error("Cannot query past events.");

      const dayEvents = schedule.filter(({ date }) => date === eventQuery);
      return dayEvents.length
      ? dayEvents.map(({ date }) => date)
      : Error(`No events found for ${eventQuery}`);
    };
  ...
  const chat= (() => {
        ...
        for (const m of muts) {
          for (const p of m.addedNodes) {
            ...
            if (
              ...
              const timeMatch = rawText.match(/(\d+)\s*minutes?/i);
              const time = timeMatch ? parseInt(timeMatch[1]) : null;
              debug.log("observer", "emit mission", {
                mission,
                location,
                isOpen,
                rawText,
              ...
              if (mission) {
                ...
                debug.log(
                  "observer",
                  "skip emit mission: no mission parsed",
                  rawText,
                );
              continue;
            }
            if (
              ...
              if (bdis.length >= 2) {
                ...
                debug.log("observer", "emit roleChange", {
                  ...
                  isOwn: bdis[0].textContent === clientName,
                  isOwnAction: bdis[1].textContent === clientName,
                });
    ...
    return (cfg = {}) => (Object.freeze({
      openChat,
      closeChat,
      send(...args){
        if(!args.length)return false;
        const msg=args.map(v=>String(v)).join(" ");
        const ignoreFocusing=cfg.ignoreFocusing??false;
        queue.push({msg,ignoreFocusing});
        sendNext();
        return true;
      },
      append(...args) {
        ...
        const items = flat
          .filter(
            (v) => v && typeof v === "object" && typeof v.text === "string",
          )
          .map((v) => ({
            user: v.user ?? "unknown",
            text: v.text,
  ...
  const outfit = (() => {
    debug.forceWarn(
      "outfit",
      "DEPRECATED;",
      "Outfit API no longer works. Stop using it.",
    );
    let wsReady = false;
    let wsSend = null;
    ...
    const setOutfit = (isInGame, outfit) => {
      debug.forceWarn(
        "outfit",
        "DEPRECATED;",
        "Outfit API are no longer work. please stop using it",
      );
  ...
  debug.group("BOOT", "System Startup");
